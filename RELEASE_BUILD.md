...
The current package smoke suite additionally starts and stops online mode on an ephemeral loopback listener with the compiler/toolchain removed from PATH. Listener readiness follows RSA/HTTP-client initialization, so this detects missing native crypto runtime support; it does not verify a real Mojang session.
