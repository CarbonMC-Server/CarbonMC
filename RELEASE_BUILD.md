...
Python 3.11+ and Rust 1.98.0 are required. `tools/release/settings.toml` pins the compiler version for release builds; the general development toolchain can remain stable. Cargo.lock is mandatory and builds use `--locked`. The packager checks the actual compiler version and native target, even when RUSTUP_TOOLCHAIN overrides rust-toolchain.toml. CI uses Python 3.12. Vendored OpenSSL requires Perl and native build tools: Strawberry Perl/nmake for Windows MSVC, Perl/Make for Linux, and MSYS Perl/Make for local Windows gnullvm. OPENSSL_SRC_PERL can select the matching executable. Release packaging fixes SOURCE_DATE_EPOCH, supplies stable compiled OpenSSL runtime directories, and suppresses OpenSSL source-file names in error records. This avoids embedding temporary Cargo install/source paths while retaining independently rebuilt native libraries. OpenSSL provider modules are disabled by the vendored build; configuration/provider environment overrides are outside the tested release configuration. The package includes the native library Apache-2.0 license. Local downloaded build tools stay under ignored work/ and are never packaged. It supports native x86_64 Windows MSVC and Linux GNU, with local Windows gnullvm as a separately labeled validation target.
...
## Encrypted-login native build validation — verified 2026-09-24

Commit `fd8c46e0fb3af76f0ae70b0588224a45ce0dc65c` passed [native release validation](https://github.com/CarbonMC-Server/CarbonMC/actions/runs/35860709097) and [standard CI](https://github.com/CarbonMC-Server/CarbonMC/actions/runs/35860709118). Both native platforms passed formatting, locked compilation, strict lint, Rust/Python tests, two independent release builds and exact archive comparison. Fresh smoke jobs passed eight package process checks and save acceptance, verifying 315 file hashes on Windows MSVC and 313 on Linux GNU.

The authentication dependency initially embedded temporary OpenSSL installation directories and MSVC error-source filenames. Diagnostic run 35826868726 isolated the differences to the executable and FILES.sha256. An independently authored forced-include header supplies stable directory macros and OPENSSL_NO_FILENAMES; CC_SHELL_ESCAPED_FLAGS ensures cc-rs correctly parses its quoted path, and SOURCE_DATE_EPOCH fixes the native timestamp. Native dependencies still compile independently; binaries are not normalized after building and exact comparison remains required.

The eight package checks include online-mode startup and clean shutdown on an ephemeral loopback listener with compiler/toolchain directories removed from PATH. Listener readiness follows RSA/HTTP-client initialization, exercising the shipped crypto provider. This does not contact Mojang or establish licensed-client acceptance, public-server security, power-loss safety, or distribution approval. Workflow artifacts remain review artifacts only.
