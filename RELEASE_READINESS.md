...
The session service in these tests is synthetic. No successful real Mojang session or licensed-client gameplay acceptance is claimed. Hosted Windows/Linux checks, lint and tests passed. Initial archive comparisons exposed native OpenSSL build-path strings. The correction at 09a4c8e is tracked in [native release validation](https://github.com/CarbonMC-Server/CarbonMC/actions/runs/35860056355) and [standard CI](https://github.com/CarbonMC-Server/CarbonMC/actions/runs/35860056345); use those run results for its native reproducibility and package-smoke evidence. Earlier failed archive runs are not release evidence. See HANDOFF.md for continuation status.
