...
The session service in these tests is synthetic. No successful real Mojang session or licensed-client gameplay acceptance is claimed. Hosted Windows/Linux checks, lint and tests passed; archive reproducibility validation exposed native OpenSSL build-path strings, and its fix still requires a passing release run; previously recorded build baselines remain historical evidence only. See HANDOFF.md for continuation status.
