# Release readiness — updated 2026-09-24
...
Local Windows gnullvm: formatting, locked workspace/all-target check, strict Clippy, 231 Rust tests (two existing subprocess helpers ignored) and ten Python packaging/save regressions passed. Ten isolated CLI/recovery invocations and online-mode startup/status/shutdown also passed. Authentication coverage includes the independent cipher/hash vectors, challenge mismatch/malformed ciphertext, cancellation-safe encrypted framing, compression inside encryption, canonical UUID/name matching, a complete encrypted configuration snapshot, failed-session non-admission, HTTPS-only requests, bounded/invalid/redirect responses, body timeout, capacity release, overall setup deadline and shutdown.

The session service in these tests is synthetic. No successful real Mojang session or licensed-client gameplay acceptance is claimed. Commit `fd8c46e` passed [standard CI](https://github.com/CarbonMC-Server/CarbonMC/actions/runs/35860709118) and [native release validation](https://github.com/CarbonMC-Server/CarbonMC/actions/runs/35860709097): Windows MSVC/Linux GNU compilation, lint/tests, independently rebuilt identical archives, and fresh-package smoke/save acceptance. Each package passed eight process checks, including online authentication initialization without compiler tools on PATH; 315 file hashes were verified on Windows and 313 on Linux. OpenSSL path/flag-quoting reproducibility failures from earlier attempts were corrected; those failed runs are not acceptance evidence. See HANDOFF.md for the next task and remaining blockers.
