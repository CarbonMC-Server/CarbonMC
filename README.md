...
- Independent Overworld, Nether, and End simulations with dimension-aware chunk palettes, lighting rules, block edits, dropped items, and persistent saves. Compact gravel pockets provide a server-authoritative ten-percent chance of flint underground in the Overworld and otherwise return gravel. Nether lava emits adjacent-chunk light; lava contact in any dimension causes damage plus brief lingering burns. Water at a player's feet or head immediately extinguishes lingering burns once direct lava contact ends; sparse soul-sand patches and basalt pillars decorate terrain away from the starter portal. A rectangular obsidian frame from 4×5 through 23×23 blocks (corners optional) remains dormant until its interior is used with flint and steel, and breaking a required obsidian edge collapses that portal interior while intact nearby frames remain active.
...
Add `localhost` in an unmodified Minecraft Java 26.2 client. The included development configuration binds only to `127.0.0.1` and uses `online_mode = false`, allowing a local client to join Carbon's starter world. `online_mode = true` now requires encrypted login and successful Mojang session verification. This path remains experimental pending licensed-client acceptance; see the authentication policy in [OPERATIONS.md](OPERATIONS.md).
...
The administrative `effect give <player> <effect> <seconds> [level]` command applies levels 1–5 for up to one hour; `effect clear <player> [effect]` removes one or all effects. Supported names are `speed`, `slowness`, `strength`, `regeneration` (or `regen`), `resistance`, `fire_resistance`, `hunger`, and `poison`. Effects show in the vanilla client HUD, survive reconnects and clean restarts, expire on server ticks, and clear on death. Fire Resistance prevents lava-contact and lingering-burn damage while active, without preventing combat damage or clearing the burn timer; damage can resume after expiry or removal. Milk clears it with other effects. Brewing and survival potion acquisition remain unfinished. Speed and Slowness currently rely on the vanilla client's movement modifier because Carbon does not yet enforce movement speed server-side.
...
Packet compression is negotiated at login with a 256-byte threshold. Wire and decompressed packet sizes and inbound rates are bounded; see [OPERATIONS.md](OPERATIONS.md). Online-mode authentication and encryption are a separate login stage described in [OPERATIONS.md](OPERATIONS.md).
